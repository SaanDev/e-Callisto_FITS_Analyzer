...
def test_runtime_requirements_include_jax_cpu():
    text = install_requirements.RUNTIME_REQUIREMENTS.read_text(encoding="utf-8")
    assert "jax[cpu]" in text
    assert "jax" in install_requirements.packages


def test_gpu_requirements_are_opt_in_and_linux_only():
    assert install_requirements.GPU_REQUIREMENTS.exists()
    text = install_requirements.GPU_REQUIREMENTS.read_text(encoding="utf-8")
    assert "jax[cuda12]" in text
    assert 'sys_platform == "linux"' in text
    # CUDA must never leak into the default install.
    runtime = install_requirements.RUNTIME_REQUIREMENTS.read_text(encoding="utf-8")
    assert "cuda" not in runtime.lower()
    assert "jax" in install_requirements.gpu_packages


def test_install_script_exposes_a_gpu_flag():
    text = (ROOT / "src" / "Installation" / "install_requirements.py").read_text(encoding="utf-8")
    assert '"--gpu"' in text
    assert "GPU_REQUIREMENTS" in text


def test_specs_bundle_jax_and_hooks_but_not_cuda():
    spec_paths = [
        ROOT / "src" / "Installation" / "FITS_Analyzer.spec",
        ROOT / "src" / "Installation" / "FITS_Analyzer_linux.spec",
        ROOT / "src" / "Installation" / "FITS_Analyzer_win.spec",
    ...
    for path in spec_paths:
        ...
        # The word appears in the explanatory comments; what must never appear
        # is a bundled CUDA package.
        assert '"jax[cuda12]"' not in text
        assert '"nvidia' not in text.lower()
        assert '"jax_cuda' not in text.lower()

    hooks = ROOT / "src" / "Installation" / "pyinstaller_hooks"
    assert (hooks / "hook-jax.py").exists()
    assert (hooks / "hook-jaxlib.py").exists()
