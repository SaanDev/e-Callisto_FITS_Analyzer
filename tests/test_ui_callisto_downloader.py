...
from src.UI.callisto_downloader import (
    ...
    _build_median_db_preview_figure,
    extract_fits_links,
...
def test_downloader_preview_renders_median_db_with_frequency_and_time_axes():
    raw = np.array([[10.0, 12.0, 30.0], [20.0, 25.0, 35.0]], dtype=np.float32)
    figure, _ax, _cbar = _build_median_db_preview_figure(
        raw,
        np.array([90.0, 80.0], dtype=float),
        np.array([0.0, 1.0, 2.0], dtype=float),
    )
    image = figure.axes[0].images[0]
    expected = (raw - np.median(raw, axis=1, keepdims=True)) * MEDIAN_DB_SCALE

    assert np.allclose(np.asarray(image.get_array()), expected)
    assert image.get_clim() == pytest.approx(MEDIAN_DB_DISPLAY_LIMITS)
...
def test_downloader_preview_renders_median_db_with_fallback_axes():
    raw = np.array([[10.0, 12.0, 30.0], [20.0, 25.0, 35.0]], dtype=np.float32)
    figure, _ax, _cbar = _build_median_db_preview_figure(raw, None, None)
    image = figure.axes[0].images[0]
    expected = (raw - np.median(raw, axis=1, keepdims=True)) * MEDIAN_DB_SCALE
