...
from src.ui.cme import cme_launcher
...
def test_build_cme_helper_command_frozen_mode(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.cme.cme_launcher._frozen_executable_path", lambda: "/tmp/app")
...
def test_launch_cme_helper_success(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.cme.cme_launcher.subprocess.Popen", lambda *_a, **_k: FakeProc())
...
def test_launch_cme_helper_quick_exit(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.cme.cme_launcher.subprocess.Popen", lambda *_a, **_k: FakeProc())
    monkeypatch.setattr("src.ui.cme.cme_launcher.time.sleep", lambda *_a, **_k: None)
