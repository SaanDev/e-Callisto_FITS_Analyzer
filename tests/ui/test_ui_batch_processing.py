...
from src.backend.radio.batch_processing import PLOTUTIL_DB_SCALE, PLOTUTIL_DISPLAY_LIMITS
from src.backend.radio.fits_io import FitsLoadResult
from src.ui.app.gui_workers import BatchProcessWorker
...
def test_batch_worker_continues_on_file_errors(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("src.ui.app.gui_workers.list_fit_files", lambda *_a, **_k: files)
    ...
    monkeypatch.setattr("src.ui.app.gui_workers.load_callisto_fits", fake_load)
    monkeypatch.setattr(
        "src.ui.app.gui_workers.build_unique_output_png_path",
        lambda out_dir, input_name: str(tmp_path / f"{input_name}.png"),
    )
    monkeypatch.setattr(
        "src.ui.app.gui_workers.save_background_subtracted_png",
...
def test_batch_worker_exports_plotutil_method_as_preconverted_db(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("src.ui.app.gui_workers.list_fit_files", lambda *_a, **_k: ["/tmp/plotutil.fit"])
    monkeypatch.setattr("src.ui.app.gui_workers.load_callisto_fits", lambda *_a, **_k: _build_result())
    monkeypatch.setattr(
        "src.ui.app.gui_workers.build_unique_output_png_path",
        lambda out_dir, input_name: str(tmp_path / f"{input_name}.png"),
    )
...
def test_batch_worker_raw_mode_does_not_call_background_subtraction(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("src.ui.app.gui_workers.load_callisto_fits", lambda *_a, **_k: _build_result())
    monkeypatch.setattr(
        "src.ui.app.gui_workers.subtract_background",
...
def test_batch_run_does_not_mutate_main_window_data(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("src.ui.radio.dialogs.batch_processing_dialog.QMessageBox.information", lambda *_a, **_k: 0)
    monkeypatch.setattr("src.ui.radio.dialogs.batch_processing_dialog.QMessageBox.warning", lambda *_a, **_k: 0)
    monkeypatch.setattr("src.ui.radio.dialogs.batch_processing_dialog.QMessageBox.critical", lambda *_a, **_k: 0)
