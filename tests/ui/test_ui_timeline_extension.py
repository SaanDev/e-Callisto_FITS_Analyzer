...
def test_extend_uses_local_files_without_touching_the_archive(window, monkeypatch):
    ...
    monkeypatch.setattr("src.backend.radio.callisto_timeline.find_archive_segment", _no_network)
...
def test_prepending_shifts_every_time_based_marking(window):
    ...
    from src.backend.session.measurements import calculate_two_point_measurement
