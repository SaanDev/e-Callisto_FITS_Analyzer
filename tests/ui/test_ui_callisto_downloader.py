...
from src.backend.radio.spectral_overview import SpectralOverviewResult
from src.ui.downloads.callisto_downloader import (
    BASE_URL,
...
def test_fetch_worker_run_uses_get_listing_when_head_probe_fails(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.build_archive_session", lambda: FakeSession())
...
def test_spectral_overview_worker_continues_after_download_failure_and_cleans_temp(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.build_archive_session", lambda: FakeSession())
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.build_spectral_overview", fake_build)
...
def test_downloader_displays_each_focus_code_in_screen_fitted_preview_tabs(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.render_spectral_overview_figure", lambda _result: Figure())
...
def test_spectral_overview_export_uses_original_figure(monkeypatch, tmp_path, extension, expected_format):
    ...
    monkeypatch.setattr(
        "src.ui.downloads.callisto_downloader.pick_export_path",
        lambda *_args, **_kwargs: (str(out_path), extension),
    )
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.QMessageBox.information", lambda *_args, **_kwargs: None)
...
def test_preview_panels_combine_what_can_combine_and_keep_the_rest_separate(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.backend.radio.burst_processor.group_combinable_paths",
    ...
    monkeypatch.setattr(
        "src.ui.downloads.callisto_downloader.load_callisto_fits", lambda *_a, **_k: FakeLoad()
    )
...
def test_preview_panels_report_a_file_that_will_not_load(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.downloads.callisto_downloader.load_callisto_fits", _boom)
