...
def test_open_github_issue_uses_open_url_robust(monkeypatch):
    ...
    monkeypatch.setattr("src.ui.help.bug_report_dialog.open_url_robust", fake_open)

    dlg = BugReportDialog(
        repo="SaanDev/e-Callisto_FITS_Analyzer",
        context_provider=lambda: {"environment": {"app_version": "2.6.0"}, "summary": {"filename": "demo.fit"}},
...
def test_generate_diagnostics_creates_bundle_and_updates_status(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "src.ui.help.bug_report_dialog.QFileDialog.getSaveFileName",
        lambda *_a, **_k: (str(target), "Zip Archive (*.zip)"),
    )
    monkeypatch.setattr("src.ui.help.bug_report_dialog.QMessageBox.information", lambda *_a, **_k: 0)
    monkeypatch.setattr("src.ui.help.bug_report_dialog.QMessageBox.warning", lambda *_a, **_k: 0)
    monkeypatch.setattr("src.ui.help.bug_report_dialog.QMessageBox.critical", lambda *_a, **_k: 0)
