...
(src/ui/gcs/gcs_fitting_window.py, src/ui/gcs/gcs_viewpoint_panel.py) and its entry
...
from src.backend.gcs.gcs_model import PARAMETER_NAMES, GCSParameters, apex_arcsec, gcs_mesh, project_to_arcsec
from src.ui.gcs.gcs_fitting_window import PANEL_LABELS, GCSFittingWindow
from src.ui.gcs.gcs_viewpoint_panel import DIFFERENCE_MODES, GCSViewpointPanel
from src.ui.gcs.gcs_viewpoint_panel import _qdatetime_utc as QDateTime
...
def test_refine_uses_every_fetched_viewpoint(window):
    from src.backend.gcs.gcs_model import gcs_mesh, project_to_arcsec
...
def test_closing_mid_download_cancels_instead_of_aborting(window):
    ...
    import src.ui.gcs.gcs_viewpoint_panel as panel_module
...
def test_a_differencing_failure_is_reported_not_swallowed(window, monkeypatch):
    ...
    panel.statusChanged.connect(messages.append)
    monkeypatch.setattr(
        "src.backend.solar.helioviewer_jp2.difference_image",
...
def test_only_jp2_coronagraph_sources_are_offered(window):
    from src.backend.solar.helioviewer_jp2 import GCS_JP2_SOURCES
...
def test_the_fetch_worker_hands_back_the_sequence(monkeypatch):
    ...
    from src.ui.gcs.gcs_viewpoint_panel import JP2FetchWorker
