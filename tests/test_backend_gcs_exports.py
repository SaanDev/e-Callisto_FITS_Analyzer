...
(src/Backend/figure_export.py), the viewpoints and height–time figures
(src/Backend/gcs_figures.py) and the PDF report (src/Backend/gcs_report.py).
"""
...
def _report_input(**overrides):
    from src.Backend.gcs_model import GCSParameters, ObserverGeometry
    from src.Backend.gcs_report import GCSReportInput, ViewpointInfo
...
def test_report_pdf_holds_every_section_and_its_symbols(tmp_path):
    ...
    from src.Backend.gcs_report import generate_gcs_report_pdf
...
def test_report_without_fits_says_how_to_get_kinematics():
    from src.Backend.gcs_report import Text, build_report_blocks
...
def test_formal_errors_are_shown_only_for_the_refined_parameters():
    ...
    from src.Backend.gcs_report import gcs_parameter_pairs
