...
def test_normalize_version():
    ...
    assert update_checker.normalize_version("2.6.0-dev") == (2, 6, 0)
    assert update_checker.normalize_version("invalid") == ()


def test_is_newer_version():
    assert update_checker.is_newer_version("2.0", "2.2.1")
    assert update_checker.is_newer_version("2.2.1", "2.6.0-dev")
    assert not update_checker.is_newer_version("2.6.0-dev", "2.2.1")
...
def test_check_for_updates_skips_prerelease_for_platform(monkeypatch):
    releases_payload = [
        {
            "tag_name": "v2.6.0-dev-beta(Linux)",
            "name": "Linux beta",
            "prerelease": True,
            "draft": False,
            "assets": [
