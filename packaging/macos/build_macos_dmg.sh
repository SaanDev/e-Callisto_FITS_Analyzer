...
# Optional overrides:
#   ROOT=/path/to/repo VERSION=3.1.0 PYTHON_BIN=/path/to/python3 \
...
DMG_SETTINGS="$ROOT/packaging/macos/dmg_settings.py"
ICON_FILE="$ROOT/assets/branding/icon.icns"
SIGNER_SCRIPT="$ROOT/packaging/macos/codesign_macos_bundle.py"
OPENSSL_REPAIR_SCRIPT="$ROOT/packaging/macos/repair_macos_openssl.py"
...
Use Python 3.12+, for example:
  PYTHON_BIN=/opt/homebrew/bin/python3.13 bash packaging/macos/build_macos_dmg.sh
EOF
  exit 1
fi
...
if [ ! -f "$OPENSSL_REPAIR_SCRIPT" ]; then
  ...
  echo "Update all files under packaging/macos before building." >&2
  exit 1
fi
...
if [ "$SKIP_APP" = "1" ]; then
  ...
  echo "==> Running py2app"
  "$PYTHON_BIN" "$ROOT/packaging/macos/setup.py" py2app
...
"$PYTHON_BIN" "$ROOT/scripts/smoke_test_packaged.py" --timeout 12
