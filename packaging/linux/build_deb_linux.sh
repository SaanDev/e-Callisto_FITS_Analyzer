...
RUNTIME_REQUIREMENTS="$ROOT/requirements/requirements-runtime.txt"
BUILD_REQUIREMENTS="$ROOT/requirements/requirements-build.txt"
...
Use Python 3.11+ (recommended: 3.12 or 3.13), for example:
  PYTHON_BIN=/usr/bin/python3.13 bash packaging/linux/build_deb_linux.sh
EOF
  exit 1
fi
...
pyinstaller --clean --noconfirm "$ROOT/packaging/pyinstaller/FITS_Analyzer_linux.spec"
...
cat > "$ROOT/packaging/linux/${APP_ID}.sh" <<'EOF'
#!/bin/sh
exec /opt/e-callisto-fits-analyzer/e-callisto-fits-analyzer "$@"
EOF
chmod 755 "$ROOT/packaging/linux/${APP_ID}.sh"

cat > "$ROOT/packaging/linux/${APP_ID}.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=${APP_NAME}
Exec=${APP_ID}
...
cat > "$ROOT/packaging/linux/postinst.sh" <<'EOF'
#!/bin/sh
set -e
...
exit 0
EOF
chmod 755 "$ROOT/packaging/linux/postinst.sh"
...
fpm -s dir -t deb \
  ...
  "$ROOT/packaging/linux/${APP_ID}.sh=/usr/bin/${APP_ID}" \
  "$ROOT/packaging/linux/${APP_ID}.desktop=/usr/share/applications/${APP_ID}.desktop" \
  "$ROOT/assets/branding/FITS_analyzer.png=/usr/share/icons/hicolor/256x256/apps/${APP_ID}.png"
