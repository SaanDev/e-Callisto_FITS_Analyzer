...
- **GCS Fit — 3-D CME reconstruction:** the Graduated Cylindrical Shell flux-rope model of Thernisien et al. (2006, 2011), the same model [PyThea](https://github.com/AthKouloumvakos/PyThea) implements. Track CME and Circle Fit both measure the CME *as it appears*, so their heights and speeds are lower bounds that shrink the further the eruption sits from the plane of the sky; GCS removes that. Six parameters — propagation longitude and latitude, tilt, apex height, half angle and aspect ratio — define a "croissant" that is overlaid on the coronagraph image and adjusted until it matches the front. Drag the apex handle directly on the image (radially to raise it, tangentially to steer it) or ride the six sliders; the wireframe tracks continuously, because the whole update path is hand-rolled NumPy that costs about 0.3 ms rather than going through `SkyCoord`. **Refine fit** polishes the manual fit against points clicked along the front and reports a formal error on every parameter. Commit records the frame, and a sequence yields a **3-D de-projected** height–time plot with a true radial speed. GCS is only well posed with two or more views — from one vantage the direction trades off against height and width — so **Analysis → GCS CME Fitting…** opens a two-viewpoint window where one shell is fitted against both panels at once; with a single view the propagation direction is held fixed rather than silently fitted to a degenerate value.
...
| **GCS Fit** | Match a 3-D Graduated Cylindrical Shell to the front. The apex height is **de-projected**, so the resulting speed is the true radial one rather than a plane-of-sky lower bound. Best with two viewpoints. |
