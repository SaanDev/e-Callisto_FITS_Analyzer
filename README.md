...
- **GCS CME Fitting — 3-D reconstruction from three viewpoints:** a standalone window (**Analysis → GCS CME Fitting…** in the Solar Image Analysis window, available whether or not anything is loaded there — it fetches all three viewpoints itself) implementing the Graduated Cylindrical Shell flux-rope model of Thernisien et al. (2006, 2011), the same model [PyThea](https://github.com/AthKouloumvakos/PyThea) implements. Track CME and Circle Fit both measure a CME *as it appears*, so their speeds are lower bounds that shrink the further the eruption sits from the plane of the sky; GCS removes that. Three panels view the Sun simultaneously — the classic STEREO-A COR2 / SOHO LASCO / STEREO-B COR2 geometry, switching panel C to LASCO C3 for dates after STEREO-B was lost in 2014 — each loading **Helioviewer JPEG2000 frames** with its own colormap and contrast, and each rendering raw, running-difference or base-difference frames. JP2s are a fraction of the size of FITS, one request lists every frame in the search window, downloads are cached on disk, and each file embeds the observer position the fit needs. The loader handles the archive's traps explicitly: LASCO JP2s arrive already rotated to solar north but still carrying their original ~180° roll in the header (honouring it would put LASCO upside down under the wireframe), LASCO headers carry no observer position (SOHO is placed at L1), consecutive COR2 frames alternate between 2048 and 1024 pixels, and frames jitter by up to half a pixel — so every sequence is brought onto one co-registered grid before differencing, and differences are taken on the display values with their median offset removed rather than divided by exposure time. **One set of six sliders drives all three at once**, because the whole point is that a single shell must satisfy every viewpoint simultaneously; a single view cannot separate propagation direction from height and width, so with fewer than two the direction is held fixed rather than silently fitted to a degenerate value. Set an **event range** (UTC start and end) and it becomes every channel's range, then narrow any channel on its own; **Load all** fetches each channel's frames for its range, thinning evenly past a per-channel cap rather than cutting the event short. Every load opens in **gray running difference**. The images are laid out to use the whole window: in the **Equal** layout three square images span the full width with no axes or title rows eating into them, and the controls — Event & channels, GCS model, Display and Kinematics cards — fill exactly the space beneath; the **Focus** layout (buttons A/B/C, or keys 1–3 with an image focused) enlarges one viewpoint by about half again with the other two stacked beside it and the controls in a column. A **Solar limb** toggle outlines the photosphere at each observer's apparent radius — the reference a CME's height is judged against behind the occulter — and an **Axes** toggle restores the arcsec axes. Space plays and pauses, ←/→ step frames, and hovering reports the position angle and distance in solar radii. Drag the apex handle on whichever panel shows the front most clearly, or ride the sliders — the wireframe tracks continuously at roughly 750 updates a second across all three panels, because the projection is hand-rolled NumPy rather than `SkyCoord`. Wireframe thickness, colour, opacity and mesh density are adjustable so the grid stays readable over a noisy difference image. **Refine fit** polishes the fit against points clicked along the front in any panel and reports a formal error per parameter; **Commit GCS** records a time, and a sequence of commits yields a **de-projected 3-D** height–time profile with a true radial speed, exportable as CSV.
...
| **GCS CME Fitting** | A separate three-viewpoint window (Analysis menu). Match one 3-D Graduated Cylindrical Shell to the front in every view at once. The apex height is **de-projected**, so the speed is the true radial one rather than a plane-of-sky lower bound. |
