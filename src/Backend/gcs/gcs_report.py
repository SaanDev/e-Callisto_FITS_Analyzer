...
PDF report of a GCS fitting session (src/backend/gcs/gcs_report.py).
...
the OriginPro style of ``src.backend.common.figure_export``; the viewpoint images come
...
(``src.backend.session.project_report``), and reuses its page styles.
"""
...
def shock_parameter_pairs(params: Any, refinement: Any | None = None) -> list[tuple[str, str]]:
    ...
    from src.backend.gcs.shock_model import ELLIPSOID, SHOCK_MODEL_LABELS, shock_semi_axes
...
def build_report_blocks(data: GCSReportInput) -> list[Block]:
    ...
    from src.backend.common.figure_export import figure_png_bytes
    from src.backend.gcs.gcs_figures import FIT_ORDER_NAMES, height_time_figure
