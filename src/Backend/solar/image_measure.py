...
``src/backend/solar/coronagraph.py`` (``fit_height_time``); this module provides the
...
from src.backend.solar.coronagraph import RSUN_KM
...
def region_stats(image: np.ndarray, bounds: tuple[int, int, int, int]) -> RegionStats:
    ...
    """
    from src.backend.solar.sunpy_analysis import summarize_map_roi
