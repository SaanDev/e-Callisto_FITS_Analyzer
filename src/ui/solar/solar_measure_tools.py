...
from src.backend.solar.coronagraph import (
    HeightTimeFit,
...
from src.backend.solar.image_measure import (
    CircleFit,
    fit_circle,
    line_profile,
    region_stats,
...
class TrackingPanel(QWidget):
    ...
    def refresh_shock(self, fits: Mapping[int, Any]) -> None:
        ...
        """
        from src.backend.gcs.shock_model import SHOCK_MODEL_LABELS
    ...
    def graph_series(self) -> Any | None:
        ...
        from src.backend.gcs.gcs_figures import HeightTimeSeries
    ...
    def graph_figure(self) -> Any | None:
        ...
        from src.backend.gcs.gcs_figures import height_time_figure, height_time_title
...
class GCSParameterSlider(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        # latency. See src/backend/gcs/gcs_model.py for the measurements.
...
class GCSParameterPanel(QWidget):
    ...
    without saving anything (see ``src.backend.gcs.gcs_model``).
    """
    ...
    def __init__(self, parent=None):
        ...
        # Keep the six rows packed at the top instead of spread over whatever
        # height the card is given.
        layout.addStretch(1)
...
class ShockFitEntry(NamedTuple):
    ...
    same code. The parameters are PyThea's (see ``src.backend.gcs.shock_model``); the
...
def shock_entry_parameters(entry: ShockFitEntry) -> Any:
    """The :class:`~src.backend.gcs.shock_model.ShockParameters` a recorded fit holds."""
