...
from src.backend.solar.solar_data_analysis import (
    AiaArrayMap,
    AiaCompositeSpec,
    AiaMovieExportSpec,
    AiaRegion,
...
from src.backend.solar.calibration_levels import (
    LevelOption,
    LevelResult,
    apply_level,
    detect_frame_level,
...
from src.backend.solar.derotation import (
...
from src.backend.services.download_manager import format_bytes, format_eta
from src.backend.solar.image_measure import ruler_measurement
...
from src.backend.solar.hmi_vector_field import (
...
from src.backend.solar.jsoc_client import (
    SIZE_BIN2,
    SIZE_BIN4,
    SIZE_CUTOUT,
    SIZE_FULL,
...
from src.backend.solar.sunpy_archive import (
    DATA_KIND_MAP,
    SunPyFetchResult,
    SunPyLoadResult,
    SunPyQuerySpec,
...
from src.ui.downloads.download_queue_panel import DownloadProgressPanel
from src.ui.common.font_utils import preferred_monospace_font_family
...
class CompositeBuildWorker(QObject):
    ...
    def _layer_sources(self, spec: Any, base_times: list[Any]) -> tuple[list[str], list[Any]]:
        ...
        time, in :meth:`_run`.
        """
        from src.backend.solar.sunpy_archive import build_spec_for_observable, fetch, search
    ...
    def _load_one_layer_frame(self, path: str, spec: Any) -> Any:
        ...
        from src.backend.solar.solar_data_analysis import extract_map_frames
        from src.backend.solar.sunpy_archive import DATA_KIND_MAP, load_downloaded
    ...
    def _frame_geometry(frame: Any) -> tuple:
        ...
        from src.backend.solar.coronagraph import solar_center_from_meta
        from src.backend.solar.coronagraph_composite import rsun_pixels
    ...
    @Slot()
    def run(self):
        from src.backend.solar.lasco_ingest import quiet_lasco_metadata_noise
    ...
    def _run(self):
        try:
            ...
            from src.backend.solar.coronagraph import solar_center_from_meta
            from src.backend.solar.coronagraph_composite import (
                composite_frame,
                layer_alpha,
                match_layer_frames,
                rsun_pixels,
            )
            from src.backend.solar.multiview import observer_separation_deg, reproject_map_to
...
class VectorFieldDownloadWorker(QObject):
    ...
    def run(self):
        try:
            from src.backend.services.download_manager import DownloadItem, DownloadManager
            from src.backend.solar.jsoc_client import JsocEmptyRecordSetError, export_hmi_vector_urls
...
class SolarDataAnalysisWindow(PfssControlsMixin, QMainWindow):
    ...
    def _connect_signals(self):
        ...
        self.gcs_fitting_action.triggered.connect(self.open_gcs_fitting_window)
    ...
    def _sync_menu_action_state(self, *, loaded: bool) -> None:
        ...
        # GCS fitting is deliberately never gated: it is a standalone window that
        # fetches all three viewpoints itself, on its own threads, so it needs
        # neither loaded frames here nor an idle analyzer. Frames loaded here are
        # only ever an optional seed for its first panel.
        self.gcs_fitting_action.setEnabled(True)
    ...
    def open_helioviewer_preview(self):
        ...
        try:
            from src.ui.solar.helioviewer_preview_dialog import HelioviewerPreviewDialog
    ...
    def _nrgf_filter_frame(self, frame: Any, current: np.ndarray) -> np.ndarray:
        ...
        try:
            from src.backend.solar.coronagraph import nrgf, solar_center_from_meta
    ...
    def build_hi_jmap(self) -> None:
        ...
        from src.backend.solar.hi_jmap import build_jmap, subtract_background
        from src.backend.solar.coronagraph import solar_center_from_meta
        from src.ui.solar.solar_measure_tools import JMapDialog
    ...
    def build_composite(self) -> None:
        ...
        from src.backend.solar.coronagraph_composite import LayerSpec, layer_config_hash
    ...
    def _restore_overlay_layers(self, saved: Any) -> None:
        ...
        """
        from src.backend.solar.coronagraph_composite import LayerSpec
    ...
    def open_multiview_dialog(self) -> None:
        ...
        if not self._map_frames:
            ...
            return
        from src.ui.solar.multiview_dialog import MultiViewpointDialog
    ...
    def open_gcs_fitting_window(self) -> None:
        """Open the standalone three-viewpoint GCS fitting window."""
        # Lazy import, as every heavy window here is: it keeps this window's own
        # import cheap. No packaging entry is needed — the specs bundle 'src.ui'
        # as a whole package (FITS_Analyzer.spec:112).
        from src.ui.gcs.gcs_fitting_window import GCSFittingWindow

        existing = getattr(self, "_gcs_window", None)
        try:
