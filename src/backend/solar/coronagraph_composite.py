...
* **Viewpoint.** Layers are reprojected onto the base image's WCS by the caller
  (see :mod:`src.backend.solar.multiview`). For disk emission the standard
...
Pixel convention matches :mod:`src.backend.solar.coronagraph`: images are indexed
...
def rsun_pixels(frame: Any, data_shape: tuple[int, int] | None = None) -> float:
    ...
    :func:`src.backend.solar.coronagraph.pixel_radius_to_rsun` inverts. Falls back to
...
def layer_rgb(data: Any, spec: LayerSpec, visible: Any = None) -> np.ndarray:
    ...
    """
    from src.backend.solar.solar_data_analysis import apply_display_scale, array_to_rgb_uint8
...
def composite_frame(
    ...
    *,
    ...
    the caller reprojects them (see :mod:`src.backend.solar.multiview`).
    """
