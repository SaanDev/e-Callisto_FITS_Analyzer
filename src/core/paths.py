...
def source_project_root() -> Path:
    """Return the checkout root independently of a caller's package depth."""
    return Path(__file__).resolve().parents[2]


def source_main_path() -> Path:
    """Return the source launcher used by helper subprocesses."""
    return source_project_root() / "src" / "main.py"
...
def project_base_path(
    *,
    ...
    return str(source_project_root())
...
def find_startup_logo_path(
    *,
    ...
    relative_candidates = [
        os.path.join("assets", "branding", "FITS_analyzer.png"),
