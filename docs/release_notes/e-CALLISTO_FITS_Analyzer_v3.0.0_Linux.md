# e-CALLISTO FITS Analyzer v3.0.0 for Linux
...
## Linux Notes
- Distributed as a Debian/Ubuntu package: `e-callisto-fits-analyzer_3.0.0_<arch>.deb`.
- Install with `sudo apt install ./e-callisto-fits-analyzer_3.0.0_<arch>.deb`.
- Run from the application menu or with `e-callisto-fits-analyzer`.
- The package installs a desktop entry and depends on common Qt/OpenGL runtime libraries such as `libgl1`, `libegl1`, `libxkbcommon-x11-0`, and `libxcb-cursor0`.
- If a Linux GPU/OpenGL stack causes startup issues, run with `CALLISTO_FORCE_SOFTWARE_OPENGL=1`.
