# e-CALLISTO FITS Analyzer v3.0.0 for macOS
...
## macOS Notes
- Distributed as a compressed disk image: `e-callisto-fits-analyzer_3.0.0_macOS_<arch>.dmg`.
- Open the DMG, then drag **e-Callisto FITS Analyzer.app** into **Applications**.
- The macOS bundle reports version `3.0.0` in its app metadata.
- This build is packaged for macOS as a desktop app bundle. If macOS Gatekeeper warns because the app is not notarized, open it from Finder using **Open** from the context menu and confirm the prompt.
